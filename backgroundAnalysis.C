...
void backgroundAnalysis()
  {
    ...
    TString infile="dis_background/DIS_10x250_Q2_1_10_Combined_Pruned.root";
    std::string filename = infile.Data();
    std::string beam_config;
    std::string marker = "_Combined_Pruned.root";

    size_t pos = filename.rfind(marker);
    beam_config = filename.substr(pos - 14, 14);

    size_t x_pos = beam_config.find('x');
    std::string proton_part = beam_config.substr(x_pos + 1, 3);  // e.g., "130ep"
    ...
    std::string outfilename = "outputs/backgroundAnalysisOutput_" + beam_config + ".root";
